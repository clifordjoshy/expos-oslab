...
for filename in *.xsm; do
	entry=$(grep -n $filename $HOME/myexpos/xfs-interface/inodeusertable.txt | cut -f1 -d:)
	data=$(head -$(($entry + 10)) $HOME/myexpos/xfs-interface/inodeusertable.txt | tail +$(($entry + 7)) | tr '\n' ' ')
